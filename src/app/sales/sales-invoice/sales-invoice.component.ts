...
export class SalesInvoiceComponent implements OnInit {
  ...
  constructor(
    ...
    private fb: FormBuilder, private saleService: SalesService
  ) { }
    ...
    this.saleService.getShippingDetails(transactionID).subscribe( res => {
      if (res && res.status === '200') {
        ...
        if (this.shippingDetailList.length > 0) {
          this.shippingForm.controls['transactionID'].setValue(this.shippingDetailList[0].transactionID);
          if (this.shippingDetailList[0].ledgerID !=0) {
            this.shippingForm.controls['ledgerID'].setValue(this.shippingDetailList[0].ledgerID);
          }
          this.shippingForm.controls['transporterLRNO'].setValue(this.shippingDetailList[0].transporterLRNO);
  ...
  updateShippingData() {
    if (this.shippingForm.valid) {
      for( var i = 0; i < this.formData.length; i++) {
        this.formData[i].ledgerID = this.shippingForm.controls['ledgerID'].value;
        this.formData[i].transporterLRNO = this.shippingForm.controls['transporterLRNO'].value;
        this.formData[i].transactioneWayBillNo = this.shippingForm.controls['transactioneWayBillNo'].value;
        this.formData[i].transporterLRDate = this.shippingForm.controls['transporterLRDate'].value;
        this.formData[i].transactioneWayBillDate = this.shippingForm.controls['transactioneWayBillDate'].value;
        this.formData[i].distanceKMS = this.shippingForm.controls['distanceKMS'].value;
        this.formData[i].vehicleNo = this.shippingForm.controls['vehicleNo'].value;
      }
      ...
      if (res && res.status === '200') {
        ...
        for (let key in data) {
          if (data.hasOwnProperty(key)) {
            this.shippingLedgerList.push({'label': data[key].ledgerName, 'value': data[key].ledger_ID});
          }
        }
      }
    });
