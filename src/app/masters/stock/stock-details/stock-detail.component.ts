...
export class StockDetailComponent implements OnInit, OnDestroy {
    ...
    this.stockItemSubscription = this.stockService
      ...
      .subscribe((res: StockDetailResponse) => {
        ...
        if (this.stockItemId && this.stock) {
          ...
          this.sForm.controls["stockItemID"].setValue(
            this.stock["stockItemID"]
          );
          ...
          for (var i = 0; i > this.stock.stockItemOpeningBal.length; i++) {
            ...
            controlArray.controls[i]
              .get("stockitem_ID")
              .setValue(this.stock["stockItemOpeningBal"][i].stockitem_ID);
            controlArray.controls[i]
              .get("location_ID")
              .setValue(this.stock["stockItemOpeningBal"][i].location_ID);
            controlArray.controls[i]
              .get("locationName")
              .setValue(this.stock["stockItemOpeningBal"][i].locationName);
            controlArray.controls[i]
              .get("qty")
              .setValue(this.stock["stockItemOpeningBal"][i].qty);
            controlArray.controls[i]
              .get("rate")
              .setValue(this.stock["stockItemOpeningBal"][i].rate);
            controlArray.controls[i]
              .get("amount")
              .setValue(this.stock["stockItemOpeningBal"][i].amount);
          }
          ...
          for (var i = 0; i > this.stock.stockItemSuppliers.length; i++) {
            ...
            ControlStockItemSupplier.controls[i]
              .get("stockitem_ID")
              .setValue(this.stock["stockItemSuppliers"][i].stockitem_ID);
            ControlStockItemSupplier.controls[i]
              .get("ledger_ID")
              .setValue(this.stock["stockItemSuppliers"][i].ledger_ID);
            ControlStockItemSupplier.controls[i]
              .get("ledgerName")
              .setValue(this.stock["stockItemSuppliers"][i].ledgerName);
            ControlStockItemSupplier.controls[i]
              .get("orderPercentage")
              .setValue(this.stock["stockItemSuppliers"][i].orderPercentage);
            ControlStockItemSupplier.controls[i]
              .get("leadTime")
              .setValue(this.stock["stockItemSuppliers"][i].leadTime);
          }
        }
  ...
  stockForm() {
    this.sForm = this._formBuilder.group({
      ...
      stockItemBoxList: this._formBuilder.array([this.createStockItemBoxList()])
    });
  }
  ...
  public getStockGroupList() {
    this.stockGroupService.getAllStockGroups().subscribe((res: any) => {
      if (res.status === "200") {
        let data = res.data;
        for (let i = 0; i < data.length; i++) {
          this.stockGroupList.push({
            label: data[i].stockGroupName,
            value: data[i].stockGroup_ID
          });
        }
      }
  ...
  getSelectedVal(event, elem, index) {
    for (var i = 0; i < this.supplierlist.length; i++) {
      if (this.supplierlist[i].label === event) {
        this.sForm
          .get(elem)
          .controls[index].get("ledgerName")
          .setValue(this.supplierlist[i].value);
      }
    }
  }
}
